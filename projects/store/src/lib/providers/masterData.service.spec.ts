...
const flushData = {
  viewPointCategories: [viewCategoryData],
  transportationCategories: [transportationCategoryData],
  cities: [cityData],
};
...
describe('masterData test', () => {
  beforeEach(async () => {
    ...
    viewPointCatService.all$
      .pipe(zip(transportationCategoryService.all$, cityService.all$))
      .subscribe(value => {
        result = value;
      });
  });
