...
export function travelAgendaReducer(
  state = INIT_UI_TRAVELAGENDA_STATE,
  action: UITravelAgendaAction,
): ITravelAgendaUI {
  if (
    !action.payload ||
    action.payload.entityType !== EntityTypeEnum.TRAVELAGENDA
  ) {
    return state;
  }
