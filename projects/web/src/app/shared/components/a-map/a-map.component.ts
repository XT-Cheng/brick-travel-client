...
export class AMapComponent implements AfterViewInit, OnDestroy {
  ...
  //#region Event
  @Output()
  viewPointClickedEvent: EventEmitter<IViewPointBiz>;
  @Output()
  viewPointAddedToDailyTrip: EventEmitter<{
    dailyTrip: IDailyTripBiz;
    added: IViewPointBiz;
  }>;
  @Output()
  viewPointRemovedFromDailyTrip: EventEmitter<{
    dailyTrip: IDailyTripBiz;
    travelAgenda: ITravelAgendaBiz;
    removed: ITravelViewPointBiz;
  }>;
  @Output()
  pointChoosedEvent: EventEmitter<AMap.LngLat>;
  //#endregion
  ...
  minHeight: number;
  @Input()
  allowSelectPoint = false;

  @Input()
  set viewMode(viewMode: boolean) {
    this._viewMode = viewMode;

    this._markers.forEach(marker => {
      marker.windowComponent.instance.actionAllowed = this.actionAllowed(
        marker.viewPoint,
      );
  ...
  @Input()
  set pointChoosed(choosed: AMap.LngLat) {
    if (choosed) {
  ...
  protected set city(city: ICityBiz) {
    if (city) {
      this._city = city;
      if (this._map == null || this._markers.size > 0) {
        return;
      }
  ...
  @Input()
  protected set selectedViewPoint(vp: IViewPointBiz) {
    if (!vp) {
      return;
    }
  ...
  constructor(
    ...
    this._viewPointMarkerFactory = this._resolver.resolveComponentFactory(
      ViewPointMarkerComponent,
    );
    this._informationWindowFactory = this._resolver.resolveComponentFactory(
      InformationWindowComponent,
    );

    this.viewPointClickedEvent = new EventEmitter<IViewPointBiz>();
    this.viewPointAddedToDailyTrip = new EventEmitter<{
      dailyTrip: IDailyTripBiz;
      added: IViewPointBiz;
    }>();
  ...
  ngAfterViewInit(): void {
    ...
    if (this.allowSelectPoint) {
      this._mapClickListener = AMap.event.addListener(
        this._map,
        'click',
        ($event: any) => {
  ...
  private generateDailyTrip() {
    ...
    for (const key of Array.from(this._markers.keys())) {
      this.updateMarkerInfor(
        this._markers[key],
        this._markers[key].viewPoint,
        this.actionAllowed(this._markers[key].viewPoint),
        false,
        -1,
      );
    }
    ...
    this.destroyExtraMarkers(
      this._dailyTrip.travelViewPoints.map(tvp => tvp.viewPoint.id),
      false,
    );

    const viewPoints = (<ITravelViewPointBiz[]>(
      this._dailyTrip.travelViewPoints
    )).map(tvp => tvp.viewPoint);
    ...
    if (!this._pointChoosedMarker) {
      ...
      const marker: AMap.Marker = new AMap.Marker({
        ...
        offset: new AMap.Pixel(
          (-1 * ViewPointMarkerComponent.WIDTH) / 2,
          -1 * ViewPointMarkerComponent.HEIGHT,
        ),
        map: this._map,
      });
    ...
    this._viewPoints.forEach(viewPoint => {
      ...
      if (!found) {
        ...
        this.updateMarkerInfor(
          ...
          viewPoint,
          this.actionAllowed(viewPoint),
          found.markerComponent.instance.inCurrentTrip,
          found.markerComponent.instance.sequence,
        );
      }
    });
  ...
  private generateLines() {
    ...
    markerInfors.sort((a, b) => {
      return (
        a.markerComponent.instance.sequence -
        b.markerComponent.instance.sequence
      );
    });
  ...
  private updateMarkerInfor(
    markerInfo: MarkerInfor,
    viewPoint: IViewPointBiz,
    actionAllowed: ActionAllowed,
    inCurrentTrip: boolean,
    sequence: number,
  ) {
    markerInfo.viewPoint = viewPoint;

    markerInfo.marker.setPosition(
      new AMap.LngLat(viewPoint.longtitude, viewPoint.latitude),
    );
    ...
    markerInfo.windowComponent.instance.actionAllowed = this.actionAllowed(
      viewPoint,
    );
  ...
  private generateMarker(
    ...
    subscriptions.push(
      crWindow.instance.viewPointClickedEvent.subscribe(vp => {
        this._markers.get(vp.id).window.close();
        this.viewPointClickedEvent.emit(vp);
      }),
    );
    subscriptions.push(
      crWindow.instance.viewPointAddedEvent.subscribe(vp => {
        this._markers.get(vp.id).window.close();
        this.viewPointAddedToDailyTrip.emit({
          dailyTrip: this.dailyTrip,
          added: vp,
        });
      }),
    );
    subscriptions.push(
      crWindow.instance.viewPointRemovedEvent.subscribe(vp => {
        this._markers.get(vp.id).window.close();

        // Remove viewpoint
        const removed = this.dailyTrip.travelViewPoints.find(
          tvp => tvp.viewPoint.id === vp.id,
        );
        this.dailyTrip.travelViewPoints = this.dailyTrip.travelViewPoints.filter(
          tvp => tvp.viewPoint.id !== vp.id,
        );
        this.viewPointRemovedFromDailyTrip.emit({
          dailyTrip: this.dailyTrip,
          travelAgenda: this.travelAgenda,
          removed: removed,
        });
      }),
    );
    ...
    const markerClickListener = AMap.event.addListener(
      marker,
      'click',
      ($event: any) => {
  ...
  private destroyExtraMarkers(remains: Array<string>, isViewPoints: boolean) {
    ...
    if (isViewPoints) {
      ...
      toBeRemoved = Array.from(this._markers.keys()).filter(key => {
        ...
        if (
          this._dailyTrip &&
          this._dailyTrip.travelViewPoints.find(tvp => tvp.viewPoint.id === key)
        ) {
          return false;
        }
