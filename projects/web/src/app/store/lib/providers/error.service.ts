...
export class ErrorService {
  ...
  public getActionError$(actionId: string): Observable<IError> {
    return this._store
      .select<IError[]>([STORE_KEY.error, STORE_ERRORS_KEY.errors])
      .pipe(
        map(errors => {
          const actionError = errors.find(error => error.actionId === actionId);
          if (actionError) {
            return Immutable(
              errors.find(error => error.actionId === actionId),
            ).asMutable();
          }
  ...
  private getLastError(store: NgRedux<IAppState>): Observable<IError> {
    return store
      .select<IError>([STORE_KEY.error, STORE_ERRORS_KEY.lastError])
      .pipe(
        map(error => {
          if (error) {
