...
import { ReuseTabCached, ReuseTabMatchMode, ReuseTabNotify, ReuseTitle } from '../../interfaces/reuse-tab/interface';
...
export class ReuseTabService implements OnDestroy {
  ...
  private _hasFilterCached: { [url: string]: boolean } = {};
  ...
  set hasFilter(value: boolean) {
    const url = this.curUrl;
    this._hasFilterCached[url] = value;
    this.di('update current tag hasFilter: ', value);
    this._cachedChange.next({
      active: 'hasFilter',
      hasFilter: value,
      list: this._cached,
  ...
  getHasFilter(url: string): boolean {
    if (typeof this._hasFilterCached[url] !== 'undefined')
      return this._hasFilterCached[url];

    return false;
  }
  clearHasFilterCached() {
    this._hasFilterCached = {};
  }
  ...
  store(_snapshot: ActivatedRouteSnapshot, _handle: any) {
    ...
    const item: ReuseTabCached = {
      ...
      hasFilter: this.getHasFilter(url),
      url,
      _snapshot,
      _handle,
    };
