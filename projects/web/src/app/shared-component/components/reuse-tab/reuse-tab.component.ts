...
import {
  ...
  ReuseTabNotify,
  ReuseTitle,
} from '../../interfaces/reuse-tab/interface';
import { ReuseTabContextService } from '../../services/reuse-tab/reuse-tab-context.service';
import { ReuseTabService } from '../../services/reuse-tab/reuse-tab.service';

@Component({
  selector: 'bt-reuse-tab',
    ...
    if (this.showCurrent) {
      ...
      if (idx !== -1 || (isClosed && notify.url === url)) {
        ...
        ls.push(<ReuseItem>{
          ...
          hasFilter: this.srv.getHasFilter(url),
          closable:
            this.allowClose &&
            this.srv.count > 0 &&
